...
void loop() {
  ...
  if(Serial.available()>0 && btconectado == true){
    ...
    dimersignal = map(intensidad_foco, 0, 255, 0, 7);
        if(dimersignal == 0 ) luz_int = 6300; 
        if(dimersignal == 1 ) luz_int = 6100;
        if(dimersignal == 2 ) luz_int = 5200;  
        if(dimersignal == 3 ) luz_int = 4900;  
        if(dimersignal == 4 ) luz_int = 4500;  
        if(dimersignal == 5 ) luz_int = 4100; 
        if(dimersignal == 6 ) luz_int = 3500; 
        if(dimersignal == 7 ) luz_int = 3000;  
    }
}
